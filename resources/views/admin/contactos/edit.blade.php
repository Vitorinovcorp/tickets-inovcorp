...
@section('title', 'Editar Contacto - Tickets Inovcorp')
            ...
            <form action="{{ route('admin.contactos.update', $contacto) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="nome" id="nome" 
                               ...
                               value="{{ old('nome', $contacto->nome) }}" required>
                        @error('nome')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="funcao_id" id="funcao_id" 
                            ...
                            @foreach($funcoes as $funcao)
                                <option value="{{ $funcao->id }}" {{ old('funcao_id', $contacto->funcao_id) == $funcao->id ? 'selected' : '' }}>
                                    {{ $funcao->nome }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="entidades[]" id="entidades" 
                            ...
                            @foreach($entidades as $entidade)
                                <option value="{{ $entidade->id }}" 
                                    {{ in_array($entidade->id, old('entidades', $contacto->entidades->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $entidade->nome }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notas_internas" id="notas_internas" 
                              ...
                              rows="4">{{ old('notas_internas', $contacto->notas_internas) }}</textarea>
                ...
                <div class="text-end">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Atualizar Contacto
                    </button>
                </div>
            </form>
        </div>
